...
parser.add_argument("--c", "--custom",
                    help="Opens the customization menu", action="store_true")
parser.add_argument("--deb", help="Debug", action="store_true")
parser.add_argument("-cd", help="Constant Click Delay",
                    action="store", type=float)
parser.add_argument(
    "-u", help="Handle update settings (auto=True/False, update)", nargs='?', const='update')
parser.add_argument(
    "-mb", help="Choose mouse button (left, right, middle)", nargs="?", const="left")
...
match args.u:
    ...
    case 'update':
        ...
        response = requests.get(
            "https://api.github.com/repos/SpamixOfficial/OpenClick/releases/latest")
        ...
        if updateinp == "y":
            ...
            print("Installing...")
            installp = subprocess.run(
                "/tmp/openclick/" + dowdirname + "/install.sh")
...
"""
        ...
        if menuinput == "--colorexamples":
            ...
            if choose_color in nocolors:
                ...
                print(Back.BLACK + Fore.LIGHTWHITE_EX + "Invalid command: \"" +
                      choose_color + "\" is not a valid color.")
        elif menuinput == "--key":
            ...
            if choose_key in hotkeynames:  # checks if the choosen key is in the key list
                ...
                print(Back.BLACK + Fore.LIGHTWHITE_EX +
                      "Invalid command: \"" + menuinput + "\" is not a command.")
        elif menuinput == "--ckey":
            for key in hotkeynames:
                print(key)
            print("\n Choose a key!")
            ...
            if choose_key in hotkeynames:  # checks if the choosen key is in the key list
                ...
                print(
                    "The key you specified either doesn't exist or it isn't supported at the time.")
    ...
    while True:
        ...
        if debinput == "--d f":
            ...
            print(Back.BLACK + Fore.LIGHTWHITE_EX +
                  "Invalid command: \"" + debinput + "\" is not a command.")
        with open(settingsfile, 'w') as outfile:
            json.dump(data, outfile, indent=4)
